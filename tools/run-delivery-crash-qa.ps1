param(
    [string]$ProfileRoot = 'E:\pzmod\test-profile-v98-delivery-crash',
    [string]$EvidenceRoot = 'E:\pzmod\evidence\v98\actual\delivery-crash'
)
...
$restartStdout = Join-Path $serverProfile 'delivery-crash-restart.stdout.log'
$restartStderr = Join-Path $serverProfile 'delivery-crash-restart.stderr.log'
...
Write-Output 'Starting isolated career-delivery crash-probe launcher'
& powershell -NoProfile -ExecutionPolicy Bypass -File (Join-Path $root 'tools\launch-multiplayer-qa.ps1') `
    -DeliveryCrashProbe -ProfileRoot $base -EvidenceRoot $evidence
if ($LASTEXITCODE -ne 0) { throw "QA launcher failed with exit $LASTEXITCODE" }

if (-not (Test-Path $serverLog)) { throw "Initial server log was not found: $serverLog" }
$initialServerOffset = (Get-Item $serverLog).Length
if (-not (Wait-LogPatternAfter $serverLog 'NLQA DELIVERY JOURNAL PARTIAL' $initialServerOffset 900)) {
    throw "Partial delivery journal marker did not arrive; inspect $serverLog"
}
Write-Output 'QA career crash probe: player-applied delivery journal observed'
$partialOffset = (Get-Item $serverLog).Length
if (-not (Wait-LogPatternAfter $serverLog 'SaveAll took' $partialOffset 180)) {
    throw "Server did not save after the partial delivery journal marker; inspect $serverLog"
}
Write-Output 'QA career crash probe: pending delivery journal reached SaveAll'
...
Write-Output "QA career crash probe: forced server stop old=$oldPid"

# Start-Process can return the short-lived Java launcher PID while the game
# process owns the cachedir. Match the real javaw command line as well as the
# recorded IDs so no stale native connection survives the hard server stop.
$clientProcesses = @(Get-CimInstance Win32_Process | Where-Object {
    $_.Name -eq 'javaw.exe' -and $_.CommandLine -and
        (($_.CommandLine -like "*$hostProfile*") -or
         ($_.CommandLine -like "*$(Join-Path $base 'mp-guest')*"))
})
$clientPids = @($clientProcesses | ForEach-Object { [int]$_.ProcessId })
...
foreach ($clientPid in ($clientPids | Sort-Object -Unique)) {
    Stop-Process -Id $clientPid -Force -ErrorAction SilentlyContinue
}
Start-Sleep -Seconds 3

# The crash probe replaces both disposable clients with fresh processes.  Do
# not arm the same-client reconnect marker here: the hands-free agent would
# intentionally disconnect the newly started host before its first snapshot.
...
$server = Start-Process $java -ArgumentList $serverArgs -WorkingDirectory $game `
...
Write-Output "QA career crash probe: server restarted old=$oldPid new=$($server.Id)"
...
Write-Output "QA career crash probe: clients restarted host=$($hostProcess.Id) guest=$($guestProcess.Id)"
...
if (-not (Wait-LogPattern $restartDebug.FullName 'NLQA DELIVERY JOURNAL RECOVERY: state=repaired' 240)) {
    throw "Pending delivery journal was not repaired after restart; inspect $($restartDebug.FullName)"
}
$hostLog = Get-ChildItem (Join-Path $hostProfile 'Logs') -Filter '*_DebugLog.txt' -File |
    Sort-Object LastWriteTime -Descending | Select-Object -First 1
if (-not $hostLog) { throw 'Host debug log was not found after career crash-probe reconnect' }
if (-not (Wait-LogPattern $hostLog.FullName 'CAREER DELIVERY RECOVERY SNAPSHOT' 240)) {
    throw "Fresh host did not observe the repaired career snapshot; inspect $($hostLog.FullName)"
}
if (-not (Wait-LogPattern $hostLog.FullName 'CAREER DELIVERY RECOVERY RESULT: item=Base.RippedSheets count=[89][0-9]* source=server-repair' 180)) {
    throw "Fresh host did not observe at least eight restored sheets in its real inventory; inspect $($hostLog.FullName)"
}
Write-Output "QA career crash probe: old=$oldPid new=$($server.Id)"
Write-Output 'PASS: partial career delivery transaction repaired after forced server crash'
