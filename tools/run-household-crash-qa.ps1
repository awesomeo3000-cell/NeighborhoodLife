param(
    [string]$ProfileRoot = 'E:\pzmod\test-profile-v97-household-crash',
    [string]$EvidenceRoot = 'E:\pzmod\evidence\v97\actual\household-crash'
)
...
$restartStdout = Join-Path $serverProfile 'household-crash-restart.stdout.log'
$restartStderr = Join-Path $serverProfile 'household-crash-restart.stderr.log'
...
Write-Output 'Starting isolated household crash-probe launcher'
& powershell -NoProfile -ExecutionPolicy Bypass -File (Join-Path $root 'tools\launch-multiplayer-qa.ps1') `
    -HouseholdCrashProbe -PreserveHousehold -ProfileRoot $base -EvidenceRoot $evidence
...
if (-not (Wait-LogPatternAfter $serverLog 'NLQA HOUSEHOLD JOURNAL PARTIAL' $initialServerOffset 600)) {
    throw "Partial household journal marker did not arrive; inspect $serverLog"
}
$partialOffset = (Get-Item $serverLog).Length
Write-Output 'QA household crash probe: partial player-applied journal phase observed'
if (-not (Wait-LogPatternAfter $serverLog 'SaveAll took' $partialOffset 180)) {
    throw "Server did not save after the partial household journal marker; inspect $serverLog"
}
Write-Output 'QA household crash probe: pending journal reached SaveAll'
...
Write-Output "QA household crash probe: forced server stop old=$oldPid"

Set-Content (Join-Path $hostProfile 'reconnect-request') 'household-crash-restart'
...
Write-Output "QA household crash probe: server restarted old=$oldPid new=$($server.Id)"
...
Write-Output "QA household crash probe: clients restarted host=$($hostProcess.Id) guest=$($guestProcess.Id)"
...
if (-not (Wait-LogPattern $restartDebug.FullName 'NLQA HOUSEHOLD JOURNAL RECOVERY: state=repaired' 180)) {
    throw "Pending household journal was not repaired after restart; inspect $($restartDebug.FullName)"
}
if (-not (Wait-LogPatternAfter $hostLog.FullName 'HOUSEHOLD JOURNAL RECOVERY RESULT: Household storage recovery repaired' $hostOffset 180)) {
    throw "Host did not observe the repaired household result; inspect $($hostLog.FullName)"
}
Write-Output "QA household crash probe: old=$oldPid new=$($server.Id)"
Write-Output 'PASS: partial household storage transaction repaired after forced server crash'
