param(
    [string]$ProfileRoot = 'E:\pzmod\test-profile-v102-partnership',
    [string]$EvidenceRoot = 'E:\pzmod\evidence\v102\actual\partnership'
)
...
& pwsh -NoProfile -ExecutionPolicy Bypass -File `
    ...
    -PartnershipProbe -ProfileRoot $base -EvidenceRoot $evidence
...
$serverPattern = 'NLQA PARTNERSHIP SEED: target=marisol dates=2 trust=30 attraction=30 friendship=40'
if (-not (Wait-LogPattern $serverLog $serverPattern 180)) {
    throw "Partnership seed did not arrive; inspect $serverLog"
}
...
$hostPattern = 'PARTNERSHIP HOST RESULT: target=marisol status=Partner exclusive=true isPartner=true'
$guestPattern = 'PARTNERSHIP GUEST RESULT: target=marisol status=Unavailable exclusive=true isPartner=false'
$rejectPattern = 'PARTNERSHIP GUEST REJECTION: message=Not completed: Already in a partnership\.'
if (-not (Wait-LogPattern $hostLog.FullName $hostPattern 600)) { throw 'Host partnership result did not arrive' }
if (-not (Wait-LogPattern $guestLog.FullName $guestPattern 180)) { throw 'Guest partnership result did not arrive' }
if (-not (Wait-LogPattern $guestLog.FullName $rejectPattern 180)) { throw 'Guest rejection proof did not arrive' }
...
Select-String -Path $hostLog.FullName -Pattern $hostPattern | Select-Object -Last 1 | ForEach-Object Line |
    Set-Content (Join-Path $evidence 'host-partnership-result.txt')
Select-String -Path $guestLog.FullName -Pattern ($guestPattern + '|' + $rejectPattern) | ForEach-Object Line |
    Set-Content (Join-Path $evidence 'guest-partnership-result.txt')
"PASS: actual host-plus-guest partnership snapshot and rejection loop completed" |
    Set-Content (Join-Path $evidence 'RESULT.txt')
Write-Output 'PASS: actual host-plus-guest partnership snapshot and rejection loop completed'
Stop-IsolatedProcesses
