...
function NLJournal:new(index)
    ...
    o.playerIndex = index
    o.backgroundColor = { r=0.055, g=0.095, b=0.13, a=0.98 }
    o.borderColor = { r=0.3, g=0.8, b=0.68, a=1 }
...
function NLJournal:initialise()
    ...
    for i,id in ipairs(NLDefinitions.careerOrder) do
        self:button(16+(i-1)*150,48,140,NLDefinitions.careers[id].name,"select",id)
    end
    self.deliverButtons = {}
...
function NLJournal:prerender()
    ...
    self:drawText("NEIGHBORHOOD / CAREER JOURNAL",16,14,0.8,1,0.92,1,UIFont.Small)
    local p = NLClient.profiles[self.playerIndex]
    if not p then
        self:drawText("Waiting for world data. Try Refresh.",16,95,1,1,1,1,UIFont.Small)
    ...
    self:drawText(def.ranks[progress.rank] .. " | Skill " .. p.skill .. " | Career XP " .. progress.xp,
        16,90,1,1,1,1,UIFont.Small)
    self:drawText("Community credits: " .. p.credits .. " | Deliveries: " .. progress.delivered,
        16,115,0.75,0.85,0.9,1,UIFont.Small)
    self:drawText("SUPPLY REQUESTS / refresh each world day",16,142,0.8,1,0.92,1,UIFont.Small)
    for i,c in ipairs(NLDomain.contracts(p)) do
        local y = 170+(i-1)*48
        local done = p.claimed[c.id]
        self:drawText(c.amount .. " x " .. c.item .. (done and " / DONE" or ""),16,y,1,1,1,1,UIFont.Small)
        self.deliverButtons[i]:setEnable(not done)
    end
    self:drawText("Delivery consumes items in your main inventory.",16,303,0.95,0.82,0.6,1,UIFont.Small)
    ...
    self:drawText(text,16,366,0.75,0.85,0.9,1,UIFont.Small)
    -- Wrap status rather than drawing arbitrarily long server feedback off-panel.
    local message = p.message or ""
    self:drawText(string.sub(message,1,72),16,392,0.8,1,0.92,1,UIFont.Small)
    if #message>72 then self:drawText(string.sub(message,73,144),16,411,0.8,1,0.92,1,UIFont.Small) end
end
