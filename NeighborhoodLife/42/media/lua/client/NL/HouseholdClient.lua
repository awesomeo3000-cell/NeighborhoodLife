    ...
    if command == "invite" then
        for i = 0, getNumActivePlayers() - 1 do
            ...
            if player then
                ...
                old.invite = args
                old.message = "Household invitation received"
                NLHouseholdClient.snapshots[i] = old
            end
        end
        return
    end
...
function NLHouseholdClient.request(index, command, args)
    ...
    if isClient() then
        sendClientCommand(player, "NeighborhoodHousehold", command, args or {})
    elseif NLHouseholdAuthority then
        NLHouseholdAuthority.command("NeighborhoodHousehold", command, player, args or {})
    end
end
