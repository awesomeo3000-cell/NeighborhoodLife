...
Each authored neighbor now has a server-clock-driven home/work routine. The
routine is stored with the persistent NPC row, selects a saved-home-relative
career destination during the 08:00–17:00 work window, continues a small home
route outside that window, and travels through the existing authoritative
native-body/fallback movement loop. Presence heartbeats replicate the current
routine and career label to each client. Lua 5.1 mocks, the installed-game
...
The production first-slice menu now routes `ask_work`, `talk_home` and
`compliment` through the existing server-authoritative social command. Marisol,
Kenji and Amara each provide personality-specific responses. The relationship
record persists `workTalks`, `homeTalks`, `compliments` and `lastCompliment`,
with friendship/trust/attraction changes and the existing pacing rules. The
same menu now conditionally exposes partnership, breakup and apology actions
from authoritative relationship flags.

The Lua 5.1 mock suite, installed-game Kahlua engine-VM suite and direct menu
contract pass separately. The fresh hands-free Build 42.20.4 capture in
`evidence/v138/actual/social-breadth/` records the real host action sequence and
the real guest's replicated event sequence. This closes the tested social
breadth slice only; it does not close broader conversation/romance coverage or
the native NPC server-body reannouncement gate. QA helpers and profiles remain
outside the production package.

## v1.96 actual global ModData crash recovery

The fresh hands-free Build 42.20.4 host-plus-guest capture in
`evidence/v137/actual/global-journal-2/` drives the production household
`task` command with a QA-only fault that leaves the shared world journal in its
`before-clear` state. The runner waits for the real dedicated server's
`SaveAll`, forcibly stops that server, restarts it, and launches fresh host and
guest clients against the same isolated save. The restarted server records
`NLQA GLOBAL JOURNAL RECOVERY: state=repaired`, and the fresh host records
`Global data recovery repaired` after its production refresh. This is actual
gameplay/engine evidence, not a mock or Kahlua-only result. It closes the
tested global ModData crash-atomicity gap; broader offline/co-op household
routines and native NPC server-body reannouncement remain open.
...
`evidence/v132/actual/native-roster-setter/` confirmed that
`IsoPlayer.setLocalPlayer(1, body)` writes a temporary body into the static
local-player array (`setter=called setterContains=true`). A fresh native list
still reads four, and both clients remain `source=qa-local-replica`, confirming
that the static local-player array is not the server's per-connection roster.
This is actual installed-game diagnostic evidence, not native replication
completion. The remaining bridge target is the per-`UdpConnection` player array
and `GameServer.sendPlayerConnected`; the probe stays QA-only and outside the
production package.
...
The fresh isolated Build 42.20.4 capture in
`evidence/v126/actual/neighborhood-slice/` completes the first host-plus-guest
neighborhood slice through the production paths. The host receives Marisol's
`Partner` result, the guest receives `Unavailable` plus the authoritative
`Not completed: Already in a partnership.` rejection, and the host completes a
direct NPC context callback. The same run completes the medic promotion,
shared-home invite/accept, storage deposit and withdrawal, ownership transfer,
and tidy-home reward while the server records repeated production NPC path
updates and both clients render the three named NPC replicas.

This is actual installed-game multiplayer evidence, not mock or Kahlua
engine-VM evidence. The QA-only guest stimulus waits past the production
social anti-spam window before issuing the second partnership request. Native
server-body reannouncement remains open because the v1.87 bridge probe still
found no usable Lua `GameServer` bridge.

## v1.87 native NPC bridge probe

The fresh hands-free Build 42.20.4 host-plus-guest capture in
`evidence/v124/actual/native-roster/` observed `IsoPlayer.getPlayers()` with
four native bodies, temporary server-roster mutation from `before=2` to
`after=5`, and successful server-side sync/visual helper calls. The fresh
clients still identified the rendered body as `source=qa-local-replica`; direct
class-loader and `Class.forName` access from the real server Lua bridge both
raised `java.lang.RuntimeException`. `RESULT.txt` therefore records a
diagnostic compatibility result, not native multiplayer completion. This
narrows the remaining native reannouncement work to an exposed engine bridge
or supported server API.

## v1.86 actual NPC date persistence gate

The isolated hands-free Build 42.20.4 capture in `evidence/v122/actual/date-persistence-24/`
completed Marisol's production two-step date activity on the real host, observed
`DATE ACTIVITY SNAPSHOT: status=completed completedDates=1`, and recorded the
engine save hook as `NLQA DATE SERVER SAVE: ok=true result=nil`. The verifier then
stopped the dedicated server, restarted it against the same isolated cachedir,
and launched fresh host and guest clients. The restarted server recorded
`NLQA DATE SEED SKIPPED: persisted completedDates=1`; both fresh clients
reconnected. `RESULT.txt` reports the actual server reload gate. This is actual
gameplay/engine evidence, distinct from mock tests and installed-game Kahlua
VM tests. The client-side reload announcement is QA-only; native NPC server-body
reannouncement remains open because the real Lua bridge still exposes
`GameServer=nil`.
...
| Persistent neighborhood NPCs | Production `NpcAuthority` creates all three authored vertical-slice neighbors (Marisol, Kenji and Amara) as native `IsoPlayer` bodies, repairs legacy stacked saved rows to distinct free squares, gives each identity/outfit data and a persisted nearby route, restores the exact fractional position across a dedicated-server restart, sends immediate and periodic `npc_presence` from a dedicated server, and production clients render named revision-checked native replicas with plumbobs and disconnect/menu cleanup; v1.10 proves the same client reconnects after the dedicated server restarts, v1.11 proves distinct native positions on both clients, v1.13 drives client replicas through Build 42's native `preupdate`/`update`/path behavior frame with a bounded interpolation fallback, v1.27 adds a best-effort `GameServer` reannouncement adapter plus mock contract coverage, v1.31 makes the dedicated-server fallback collision-aware and reroutes a blocked waypoint instead of stepping through it, v1.32 retires native deaths into persistent dead rows and schedules saved-tile recovery for missing streamed bodies, v1.47 adds loaded-cell zombie detection plus server-authoritative retreat/pause behavior, v1.48 drops stale client NPC handles before recreating a streamed-out native replica, and v1.49 proves that recovery in a live host+guest Build 42 session, v1.51 promotes a late-arriving server-native body over an existing compatibility replica, v1.53 reconciles an engine-native body from the loaded cell even when no new mod presence packet arrives, and v1.57 adds a native online-id hint to loaded-cell promotion, v1.59 suppresses duplicate native hints, and v1.60 reserves stable authored online-id slots for the three vertical-slice identities and applies available hints to compatibility replicas, and v1.75 adds a 30-tick motion heartbeat carrying each active server route target so compatibility replicas continue moving between authoritative samples; v1.76 records shared household activity totals and per-member home-aspiration milestones in authoritative snapshots; v1.61 tries method/field assignment with getter verification and records the actual dedicated-server bridge result; v1.77 probes the exposed `IsoPlayer` static surface and proves that mutating the `getOnlinePlayers()` copy does not reach a client-native NPC body; v1.86 proves the completed Marisol date state is saved by the real dedicated server and reloaded after a restart while fresh host and guest clients reconnect; v1.88 verifies host partnership, guest isolation and the authoritative rejection in the full neighborhood slice; v1.97 adds personality-specific work, home and compliment actions, conditional partnership/breakup/apology menu routes, and actual host-plus-guest event replication in evidence/v138/actual/social-breadth/; v1.87 records the fresh native roster, sync-helper and blocked-reflection probe; v1.98 adds persisted server-clock home/work routines, career-relative destinations, and actual two-client home-to-work replication in evidence/v139/actual/npc-schedule/ | Build 42 native server-body reannouncement remains unproven because the real dedicated server exposes `GameServer=nil` to Lua; the v1.50 natural-walk probe reached only an 8-tile displacement and retained the NPC body, so natural engine-driven cell streaming remains open |
| NPC interaction | Server proximity/floor/visibility gates and personality-based dialogue implemented for all three native bodies; v1.7 hands-free host introduced Marisol through the production command and guest received an independent proximity-gated snapshot; v1.23 completes a real host `chat` -> `joke` sequence against Marisol during a connected host+guest run with cooldown and line-of-sight gates; v1.24 broadcasts successful social events to every connected client without sharing private relationship values; v1.26 drives a guest `introduce` against Kenji while the host remains connected and receives the guest event; v1.33 adds persistent per-NPC inventory, server-validated `give`/`request` exchange, revisioned snapshots and Relationships-panel actions, with actual host gameplay evidence; v1.34 adds normalized multi-item inventory entries, persisted display metadata and dynamic Relationships-panel selection from the player's main inventory; v1.35 proves a real dedicated-server process restart preserves the NPC inventory snapshot before a second host exchange; v1.37 adds recoverable cross-owner inventory transaction journaling and partial-state repair; v1.38 proves journal repair after a forced real server crash and reconnect; v1.83 adds a direct production world-object context menu for five social actions plus give/request, routing through the existing server-authoritative social client; v1.84 adds a relationship-profile entry and an actual hands-free host+guest callback-to-server capture; v1.85 adds a server-owned two-step date activity, active/completed state in the Relationships panel, and conditional Spend time together context routing; v1.86 verifies the completed date state survives a real dedicated-server restart with fresh host and guest connections; v1.97 adds server-authoritative work, home and compliment actions with personality-specific lines and conditional relationship-action routes, with actual host-plus-guest evidence in evidence/v138/actual/social-breadth/ | Broader world actions and item/container metadata |
| Relationships and romance | Per-player friendship/trust/attraction, bounded memories, pacing, dates and exclusive partnerships implemented; Sims-inspired panel; v1.7 actual host relation mutation and guest isolation evidence; v1.24 adds a bounded replicated social-event feed and Relationships-panel shared-event line; v1.26 proves the guest independently mutates Kenji to friendship 3/trust 2 while the host receives only the event; v1.85 adds a server-owned active date, immediate date_activity completion with bounded gains, completed-date count and conditional in-world action; v1.86 saves the completed date through the real engine save hook and reloads it after a dedicated-server restart; v1.88 verifies host partnership and guest isolation in the actual neighborhood slice; v1.97 verifies the richer social action event sequence on a real host-plus-guest session | Richer date activities, broader routines and direct two-client romance synchronization |
| Household life | v1.16 extends the v1.15 server-authoritative Neighborhood Home with persistent shared storage, exact item-type deposits from unequipped main inventory, member withdrawals, capacity and malformed-item guards, shared-storage UI actions, and real host-to-guest inventory transfer evidence; v1.44 adds owner-only transfer to an existing member with revisioned role snapshots; v1.45 makes the native storage furnishing interactable from the world-object menu with server-validated store/retrieve actions and actual host-to-guest evidence; v1.64 proves household membership, furnishing and stored `Base.RippedSheets` survive a real dedicated-server restart; v1.65 adds bounded per-instance item metadata capture, snapshot delivery, and restoration hooks plus dynamic furnishing-menu item types; v1.66 proves real host storage and guest retrieval of a damaged `Base.KitchenKnife` (condition 4) and partially filled Build 42 `Base.WaterBottle` (usedDelta 0.25) through the production furnishing callback; v1.67 adds a recoverable household storage journal that repairs a player/world half-transaction after a forced dedicated-server crash; v1.76 adds persisted shared household activity progression and per-member home-aspiration rewards; v1.96 proves repair of a shared ModData household mutation after a forced dedicated-server crash and fresh reconnect | Offline/co-op routines beyond the tested activity and wider home progression |
| Aspirations and home activities | Delivery milestones already persist and render in the career journal; v1.15 adds three authoritative home routines (tidy, meal, social) with daily replay guards and rewards; v1.76 links those shared activities to persisted per-member home-aspiration milestones and renders them in the career journal | Hobbies and broader home progression |
| Zombies optional | Careers have no kill requirements; v1.47 adds a real server-side danger scan for living zombies and bounded NPC retreat/pause behavior; v1.82 adds a deterministic host-plus-guest movement matrix for Build 42 `Zombies = 6` (None) and `Zombies = 4` with a real zombie stimulus | Broader optional-zombie gameplay beyond the tested retreat/movement loop |
| Host multiplayer | Server command adapter, private snapshots, authoritative player/NPC presence broadcasts, immediate refresh state, production client heartbeat, stale-packet rejection, dedicated-server NPC restart persistence, distinct three named NPC replicas, host and guest social mutations with private relationship state, replicated social events, server-authoritative career delivery, server-authoritative wardrobe capture plus vanilla clothing acquisition/wear and same-client reconnect snapshot persistence in v1.29, actual production saved-outfit replacement in v1.30, v1.56 actual automatic unrelated-layer removal with the connected host and guest present, vanilla client-acquisition inventory transfer, v1.33 actual host `give`/`request` exchange, v1.34 dynamic inventory metadata snapshots, server-authoritative household invite/accept/activity loop, shared-storage host deposit plus guest withdrawal, v1.66 real arbitrary-item/container metadata storage and retrieval, v1.67 actual forced-server-stop household journal repair after fresh host+guest reconnect; v1.68 actual forced-server-stop career-delivery journal repair after fresh host+guest reconnect; v1.69 actual host-plus-guest medic promotion through three production deliveries, v1.83 direct in-world NPC interaction context menu, v1.84 hands-free host+guest production callback-to-server proof, v1.85 hands-free host date-start and date-activity callbacks with guest event replication, v1.86 dedicated-server save/restart reload proof with fresh host and guest reconnection; v1.88 actual full neighborhood slice with host/guest social, career, household and direct NPC callbacks; v1.96 actual shared ModData crash-repair after fresh host reconnect; v1.97 actual richer social action routing and replicated events in evidence/v138/actual/social-breadth/; v1.87 actual native roster and blocked-reflection probe in evidence/v124/actual/native-roster/, disconnect-clean local native NPC replicas, authoritative remote marker fallback and v1.21 local native remote-player fallback, v1.27 native reannouncement adapter probing, v1.31 collision-aware native NPC fallback movement, v1.47 server-authoritative zombie danger handling in a real two-client session, v1.52 loaded-cell remote-player native discovery plus late native promotion with a hands-free host+guest capture, and v1.54 per-account appearance-select commands with separate host+guest native hair application | Simultaneous gameplay beyond the tested delivery, mod distribution, successful native body reannouncement, stable two-way remote movement under a naturally missing native peer, and replicated-character UI |
| Verification on this machine | Lua 5.1 tests; installed-game Kahlua harness; isolated real PZ profile; v1.14 vanilla world-item pickup plus fresh production delivery; v1.31 hands-free host+guest movement, NPC replicas and runtime API probe; v1.33 hands-free host `give`/`request` exchange against Marisol; v1.34 hands-free snapshot metadata and dynamic item-selection regression; v1.35 hands-free dedicated-server restart and post-restart NPC inventory exchange; v1.38 forced-crash journal repair with reconnect; v1.47 real server zombie seed, production danger response, and connected host+guest NPC replication; v1.48 stale streamed-replica contract and engine-VM coverage; v1.49 clean-profile hands-free startup plus live host+guest stale-body recovery; v1.50 natural-walk streaming attempt plus separate forced recovery probe; v1.51 late-native-body promotion contract and engine-VM coverage; v1.52 loaded-cell remote-player native discovery contract, engine-VM coverage, and actual hands-free host+guest capture; v1.53 packetless loaded-cell NPC-native promotion contract and engine-VM coverage; v1.54 appearance preset contract, panel contract, engine-VM coverage and actual hands-free host+guest native hair capture; v1.56 actual hands-free automatic wardrobe-layer removal capture; v1.57 native NPC online-id promotion contract and engine-VM coverage; v1.58 versioned evidence-root and actual host+guest probe; v1.59 duplicate native online-id suppression; v1.60 stable authored online-id assignment contract, Lua/Kahlua coverage and actual v81 host+guest probe with `onlineHints=0`; v1.61 runtime setter probe in an actual v82 host+guest run; v1.66 actual host+guest arbitrary-item/container metadata capture and restoration; v1.67 actual forced server-stop household journal repair after fresh host+guest reconnect; v1.69 actual host-plus-guest medic promotion through three production deliveries; v1.76 actual host-plus-guest shared home-aspiration progression; v1.77 actual dedicated-server `IsoPlayer` static-surface and roster-copy probe; v1.82 actual host-plus-guest NPC movement with deterministic zombies-disabled and zombies-enabled sandbox modes; v1.83 Lua/Kahlua world-context NPC interaction contract and actual vertical-slice regression; v1.84 actual host+guest direct NPC callback-to-server capture; v1.85 actual host+guest two-step NPC date activity and guest event replication in evidence/v121/actual/date2/; v1.86 actual dedicated-server save/restart reload with fresh host and guest reconnect in evidence/v122/actual/date-persistence-24/; v1.88 actual full neighborhood slice in evidence/v126/actual/neighborhood-slice/; v1.97 actual richer social action slice in evidence/v138/actual/social-breadth/ | Broader world/inventory/NPC/host integration tests and regression suite |
...
1. Broaden offline/co-op household routines; v1.76 proves shared household activity progression and its home-aspiration snapshot path; v1.63 proves a guest main-inventory retrieve from the production furnishing, v1.64 proves household storage membership/furnishing/item persistence across a dedicated-server restart, v1.66 proves arbitrary item/container metadata capture and restoration in a real host+guest run, v1.67 proves repair of an interrupted household storage transaction after a forced server stop, and v1.96 proves repair of a shared ModData household mutation after a forced server stop and fresh reconnect.
...
- Production `npc_presence` entries now include an active route target and a
  monotonic motion sequence. The dedicated server sends the stream every 30
  server ticks, while the older 120-tick cadence remains the position-save
  checkpoint.
- `NLNpcClient` follows the route target between samples through Build 42's
  native path frame, then uses the existing bounded interpolation fallback if
  that path stalls. New authoritative samples can correct the target; clients
  never author the server position.
- `tools/run-npc-movement-qa.ps1` and the `-NpcMovementProbe` launcher switch
  remain QA-only. Their real host and guest logs are actual installed-game
  evidence, separately from Lua mocks and Kahlua VM checks.
- The actual capture in `evidence/v110/actual/npc-movement-c/` passed with both
  host and guest observing a 1.68-tile Marisol displacement; the server log
  recorded changing authoritative coordinates. This proves the strengthened
  compatibility movement stream, not vanilla online-player replication.
- The native server-body reannouncement gate remains open. The real dedicated
  server still exposes `GameServer=nil`, `Java=nil`, and `getClass=nil`, so this
  milestone strengthens the verified compatibility route without claiming
  vanilla online-player replication.

## v1.76 household-linked home aspiration

- Tidy, meal and social completions now advance the same shared-home activity
  totals for every household member, while the acting member still receives
  the normal household-task credit.
- `NLAspirations` persists three home milestones (`First Nest`, `Shared Routine`,
  and `Household Heart`) in each member profile and awards each milestone once.
- Household snapshots carry `homeActivities`, `homeAspiration`, and a readable
  `homeAspirationLabel`; the client mirrors those fields into the career journal
  so a host or guest can see the current home goal.
- The QA-only `tools/run-home-aspiration-qa.ps1` captured an actual
  host-plus-guest sequence in `evidence/v111/actual/home-aspiration-e/`. The
  server recorded host tidy, guest meal and host social; the host observed
  `total=3`, `stage=3`, and `Household Heart: 3/6 household activities (+20
...
- The QA-only multiplayer launcher now writes deterministic Build 42 sandbox
  population settings for both supported modes. `-ZombiesDisabledProbe` uses
  `SandboxVars.Zombies = 6` (None) and suppresses only the QA danger stimulus;
  the production NPC authority still runs its normal loaded-cell scan.
- `tools/run-zombie-modes-qa.ps1` runs the hands-free host-plus-guest NPC
  movement loop twice from separate isolated profiles. The disabled capture
  must show both clients observing NPC motion, the server's `Zombies = 6`
  configuration, and no danger stimulus. The enabled capture must show a real
  server-created zombie and the production authoritative retreat response.
- Evidence in `evidence/v117/actual/zombie-modes/` is actual Build 42.20.4
  gameplay evidence, separate from Lua mocks and the installed-game Kahlua VM.
  QA files, sandbox profiles, and mode logs remain outside the production mod.

## v1.83 direct NPC world interaction

- Production `NLNpcInteractionMenu` discovers authored NPC bodies from world-object
  ModData or the currently rendered native replica and adds five social actions,
  a relationship-profile action, plus give/request to the in-world context menu.
- Callbacks only send the existing `NLSocialClient` commands; the server remains
  authoritative for identity, proximity, line of sight, cooldown, relationship,
  and inventory checks.
- `tests/npc-interaction-menu.lua` is a mock/Kahlua contract for discovery and
  callback routing. The hands-free host-plus-guest vertical-slice regression
  capture is in `evidence/v118/actual/npc-context-regression/`; it does not
  claim mouse-driven context-menu gameplay evidence.
- QA helpers remain outside the production package, and the native NPC
  server-body reannouncement blocker remains open.

## v1.84 direct NPC callback evidence

- The production context menu now adds `View relationship`, which opens the
  existing relationship panel for the current player without bypassing the
  server-owned relationship snapshot.
- The QA-only launcher can invoke the production menu callback hands-free for
  the host's Marisol target and the guest's Kenji target. The dedicated
  `run-npc-context-qa.ps1` runner requires both client callback markers and both
  server-authoritative social-result markers.
- Evidence in `evidence/v119/actual/direct-npc-context/` is actual Build 42.20.4
  host-plus-guest callback-to-server evidence. It does not claim a mouse-driven
  context-menu click, and QA helpers remain outside the production package.
- The native NPC server-body reannouncement blocker remains open.

## v1.85 two-step NPC date activity

- Production social state now records `activeDate` and `completedDates`. `date`
  starts an active date after the existing friendship/attraction gates; the new
  `date_activity` action completes it within a two-hour activity window, applies
  bounded friendship/trust/attraction gains, stores a bounded memory and keeps
  the existing partnership prerequisites intact.
- The Relationships panel renders the active/completed date state and exposes
  `Spend time together` only while the current NPC relation has an active date.
  The production world-object menu also conditionally exposes that callback from
  the player's authoritative social snapshot.
- `tools/run-date-qa.ps1` is a QA-only hands-free runner. The actual
  Build 42.20.4 capture in `evidence/v121/actual/date2/` observed the host's
  production `date` and `date_activity` menu callbacks, authoritative server
  results, host completion snapshot (`completedDates=1`, friendship 44, trust 41)
  and the guest's replicated `date_activity` event. This is actual multiplayer
  gameplay evidence, distinct from the Lua mock and installed-game Kahlua suites.
- QA helpers, generated profiles and evidence remain outside the production
  package; the native NPC server-body reannouncement blocker remains open.
