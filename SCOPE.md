...
| Household life | v1.16 extends the v1.15 server-authoritative Neighborhood Home with persistent shared storage, exact item-type deposits from unequipped main inventory, member withdrawals, capacity and malformed-item guards, shared-storage UI actions, and real host-to-guest inventory transfer evidence; v1.44 adds owner-only transfer to an existing member with revisioned role snapshots; v1.45 makes the native storage furnishing interactable from the world-object menu with server-validated store/retrieve actions and actual host-to-guest evidence | Offline/co-op routines beyond the tested activity, richer item metadata/container transfer |
...
| Verification on this machine | Lua 5.1 tests; installed-game Kahlua harness; isolated real PZ profile; v1.14 vanilla world-item pickup plus fresh production delivery; v1.31 hands-free host+guest movement, NPC replicas and runtime API probe; v1.33 hands-free host `give`/`request` exchange against Marisol; v1.34 hands-free snapshot metadata and dynamic item-selection regression; v1.35 hands-free dedicated-server restart and post-restart NPC inventory exchange; v1.38 forced-crash journal repair with reconnect; v1.47 real server zombie seed, production danger response, and connected host+guest NPC replication; v1.48 stale streamed-replica contract and engine-VM coverage; v1.49 clean-profile hands-free startup plus live host+guest stale-body recovery; v1.50 natural-walk streaming attempt plus separate forced recovery probe; v1.51 late-native-body promotion contract and engine-VM coverage; v1.52 loaded-cell remote-player discovery contract, engine-VM coverage, and actual hands-free host+guest capture; v1.53 packetless loaded-cell NPC-native promotion contract and engine-VM coverage; v1.54 appearance preset contract, panel contract, engine-VM coverage and actual hands-free host+guest native hair capture; v1.56 actual hands-free automatic wardrobe-layer removal capture; v1.57 native NPC online-id promotion contract and engine-VM coverage; v1.58 versioned QA evidence-root and actual host+guest probe; v1.59 duplicate native online-id suppression; v1.60 stable authored online-id assignment contract, Lua/Kahlua coverage and actual v81 host+guest probe with `onlineHints=0` | Broader world/inventory/NPC/host integration tests and regression suite |
