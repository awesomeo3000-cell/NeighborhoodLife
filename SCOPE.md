# Neighborhood Life — full scope and evidence ledger (v1.65)
...
1. Extend the actual two-client inventory slice beyond the fixed sheet exchange, verify the v1.65 metadata foundation with arbitrary items in a real host+guest run, and verify global-data crash atomicity; v1.63 proves a guest main-inventory retrieve from the production furnishing and v1.64 proves household storage membership/furnishing/item persistence across a dedicated-server restart.
